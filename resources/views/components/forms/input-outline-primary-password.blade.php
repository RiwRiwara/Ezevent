...
<div class="relative">
    <input type="{{ $type }}" id="{{ $name }}" name="{{ $name }}" aria-describedby="{{ $name }}-error" placeholder="{{ $placeholder }}" {{$attributes}} autocomplete="{{$autocomplete}}" class="block px-2.5 pb-2.5 pt-2 w-full text-md text-gray-9 bg-gray-0
    ...
    @if($type === 'password')
    <div id="togleBtn" hidden>
        <div class="absolute inset-y-0 right-0 flex items-center pr-3">
            <button type="button" class="text-gray-400 hover:text-gray-600 dark:text-gray-300 dark:hover:text-gray-500 focus:outline-none" onclick="togglePassword('{{ $name }}')">
                <svg id="hideIcon" class="w-[18px] h-[18px] text-neutral-9 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                ...
                <svg id="showIcon" hidden class="w-[18px] h-[18px] text-neutral-9 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
...
<script>
    function togglePassword(inputId) {
        ...
        if (hideIcon.hasAttribute('hidden')) {
    ...
    function hideShow(input) {
        if (input.value.trim() !== '') {
            document.getElementById('togleBtn').removeAttribute('hidden')

        } else {
            document.getElementById('togleBtn').setAttribute('hidden', true)
        }
    }
</script>